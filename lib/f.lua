...
function progress_bar(mon, x, y, length, minVal, maxVal, bar_color, bg_color)
  ...
  local barSize = math.floor((math.min(maxVal, math.max(0, minVal))/maxVal) * length)
  draw_line(mon, x, y, barSize, bar_color) --progress so far
end

function progress_bar_y(mon, x, y, height, minVal, maxVal, bar_color, bg_color)
  local barSize = math.floor((math.min(maxVal, math.max(0, minVal))/maxVal) * height)
  for i = 0, height - 1 do
    mon.monitor.setCursorPos(x, y + (height - 1) - i)
    if i < barSize then
      mon.monitor.setBackgroundColor(bar_color)
    else
      mon.monitor.setBackgroundColor(bg_color)
    end
    mon.monitor.write(" ")
